...
class HumanoidGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.obs, self.potentials, self.prev_potentials, self.velocity, self.roll, self.pitch, self.yaw, self.dof_pos, self.dof_vel)
...
@torch.jit.script
def compute_reward(obs: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, velocity: torch.Tensor,
                   roll: torch.Tensor, pitch: torch.Tensor, yaw: torch.Tensor,
                   dof_pos: torch.Tensor, dof_vel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Calculate the reward components
    velocity_reward = torch.exp(velocity) - 1.0
    roll_reward = 1.0 - torch.exp(torch.abs(roll) * -10.0)
    pitch_reward = 1.0 - torch.exp(torch.abs(pitch) * -10.0)
    yaw_reward = 1.0 - torch.exp(torch.abs(yaw) * -10.0)
    dof_pos_reward = torch.exp(dof_pos) - 1.0
    dof_vel_reward = torch.exp(dof_vel) - 1.0

    # Compute the total reward
    total_reward = velocity_reward + roll_reward + pitch_reward + yaw_reward + dof_pos_reward + dof_vel_reward
